...
**Intent.** An account carries valid AssetV1 bytes but is owned by an attacker's program; transfer must reject it.

**Outcome.** The transaction failed: `Invalid account owner`.

**Source.** [`tests/account_ownership.rs::transfer_rejects_fake_asset_owned_by_different_program`](../tests/account_ownership.rs#L140)
