...
**Intent.** The record's agent_asset does not match the asset being executed: NoApprovals.

**Outcome.** The transaction failed: `custom program error: 0x1a`.

**Source.** [`tests/execution_delegate.rs::execute_with_wrong_asset_delegate`](../tests/execution_delegate.rs#L444)

## Structured execution log

```
CPI Tree (8,194 BPF CU / 1,400,000 budget):
└── Execute FAILED: custom program error: 0x1a (8,194 / 1,400,000 CU) MplCore (no CPIs)
...
sequenceDiagram
    ...
    participant MplCore
    Delegate ->> MplCore: Execute (8194cu)
