...
**Intent.** The record's first byte is not the ExecutionDelegateRecordV1 key, so the plugin abstains: NoApprovals.

**Outcome.** The transaction failed: `custom program error: 0x1a`.

**Source.** [`tests/execution_delegate.rs::execute_with_invalid_discriminator_delegate`](../tests/execution_delegate.rs#L535)

## Structured execution log

```
CPI Tree (9,535 BPF CU / 1,400,000 budget):
└── Execute FAILED: custom program error: 0x1a (9,535 / 1,400,000 CU) MplCore (no CPIs)
...
sequenceDiagram
    ...
    participant MplCore
    Delegate ->> MplCore: Execute (9535cu)
